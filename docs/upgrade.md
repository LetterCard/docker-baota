...
本项目**不禁止**面板内更新。在面板里点了更新，新版文件会写进 `data/system/panel`
（面板代码的持久化上层）并一直保留——销毁重建也在，只是版本不再是镜像决定的。

想让面板回到镜像自带的版本：

```bash
make reset-panel CONFIRM=yes    # 只重置面板代码；配置、账号、站点、数据库全部保留
```

> 注意区分：**镜像升级要鼓励**。面板内更新后想"回退"，走上面的 reset-panel；
> 真正干净的版本演进路径永远是换镜像（lower 层自动更新，数据不受影响）。
