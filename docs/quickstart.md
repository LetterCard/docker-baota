...
- **面板版本由镜像控制，版本告警是检测而非阻断。** 若启动日志出现
  「面板实际版本（x.y.z）与镜像版本（a.b.c）不一致」，说明面板被更新过
  （新版文件已写入持久化层、屏蔽了镜像层）。容器仍会启动，但建议执行：

  ```bash
  make reset-panel CONFIRM=yes   # 把面板代码重置回镜像版本，配置与数据保留
  ```

- 在面板里点「更新 / 修复」大多会被静默拦截；个别绕过 `script/` 的路径
  （见 `docs/development.md`「禁用面板更新的防御边界」）若执行成功，就会触发上面的告警。
