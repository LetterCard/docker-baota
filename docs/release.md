...
任一架构的健康检查失败，该 job 就终止；`manifest` 合并依赖两个 job 都成功，
所以坏镜像不会出现在多架构标签里。健康检查失败会终止整个 job，登录与推送步骤根本不会执行。
...
`buildx` 支持一次调用里同时输出多种类型，但那样「推送」会发生在「健康检查」之前 ——
镜像万一有问题，它已经进 DockerHub 了（虽然没标签、拉不到，但确实在那儿）。

所以这里先构建并 `--load` 到本机、健康检查通过后再推送。第二次会命中上一次刚写入的
GHA 缓存，几乎不重新编译任何东西，代价很小；换来的是「坏镜像根本不会被推送」这个强保证。
