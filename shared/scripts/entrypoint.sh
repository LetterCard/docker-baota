...
#
#  进入本脚本时 overlay 与直通挂载已完成，此后对 /etc、/usr、/var、/www 等
#  目录的写入都会落入 /data/<同名目录>，容器销毁、重建、升级都不丢。
#
...
#    三个直通目录（站点 /www/wwwroot、MySQL 数据 /www/server/data、备份
#    /www/backup）在换镜像时被 bind 完全遮蔽，新镜像对它们没有任何写入路径，
#    动不到；会变的是 overlay 目录 —— 面板代码与默认配置自动换成新版。
#    于是升级后唯一「对不上」的地方是：新版面板代码 + 旧版面板数据库
#    （SQLite，升级时可能做 schema 迁移）。快照它，升级失败就能回到
...
#  执行的是 /baota/patch-panel.sh（镜像自有，不受持久化层影响）
# ==============================================================================
reset_panel_patches() {
    [ -x /baota/patch-panel.sh ] \
        || { warn '未找到 /baota/patch-panel.sh，跳过补丁复位'; return 0; }

    if [ "${DISABLE_PANEL_UPDATE:-true}" = 'true' ]; then
        /baota/patch-panel.sh disable-update \
...
#  面板版本一致性检测（补丁失效时的兜底）
#
#  patch-panel.sh 只能替换 script/ 目录下已知的更新入口；
#  宝塔的更新还有 API 层入口（BTPanel/__init__.py 的 upgrade_panel /
#  upgrade_env 等），不受 stub 影响。这里做最终检测：面板实际版本
#  与镜像版本不一致 = 持久化层已被污染
# ==============================================================================
audit_panel_version() {
    local expect actual
