...
本章说的是 **stable 通道**（release 通道每天自动跟进，见本章末尾）。

上游发新版时，`installStable_12.sh` 这个 URL 不变，所以 Dockerfile 不需要改；`stable/VERSION` 也不需要手工同步——**「🚀 stable：构建并发布镜像」每周一北京时间 10:00 自动运行**（也可随时手动触发），它会：

1. 🔍 下载安装脚本，从横幅提取版本号（`| 您正在安装宝塔面板 12.0.0 稳定版`）
2. ⚖️ 与 `stable/VERSION` 比对：上游更新 → 按新版本发布；一致 → 定时触发到此结束、手动触发则按原版本重新构建（横幅低于文件则告警并**绝不自动降级**）
3. 🏗️ 双架构构建 → 17 项健康检查
4. 🚀 全部通过才推送 `bugseeker/baota:<版本>`
5. ✏️ 发布成功后回写 `stable/VERSION`——文件永远对应已推送的版本，构建失败时文件不动，下次运行自动重试

也就是说 stable 通道**每周一自动跟进一次**：没有新版本就不构建，发现新版本才发布；需要「不改版本号也重建」（例如基础镜像出了安全更新）时，手动运行一次即可。定时探测失败（上游接口临时故障）时本周不会发布，下周自动重试，也可以随时手动触发。
...
**🚀 stable：构建并发布镜像**（`.github/workflows/stable-build-push.yml`）有两种触发方式：

- **定时**：每周一北京时间 10:00 自动运行。先探测上游横幅版本，与 `stable/VERSION` 比对——**有新版本才构建发布**，没有就空转结束（避免每周都重复构建、重复覆盖同一个标签）。探测失败时本周不发布，下周自动重试。
- **手动**：随时在页面触发，无论版本是否变化都会重新构建发布一次（需要「不改版本号也重建」时用这个）。

文件变更（包括推送 `stable/VERSION`）不会触发任何工作流。无论哪种触发，工作流都会先探测安装脚本实际会装的版本并校准 `stable/VERSION`（必要时回写仓库），因此即使忘了同步版本号，构建也不会因版本校验失败。

想跟进上游新版本时什么都不用做，等每周一的自动运行即可；等不及或需要重建时，手动运行本工作流——版本探测与校准内置其中，一步完成跟进与发布。
