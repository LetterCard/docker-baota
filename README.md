...
面板代码本身**不进持久化层，直接来自镜像层**。换镜像即升级面板；在面板里点「更新」不会生效（写入被 overlay 拦截），版本永远等于镜像版本。

这把面板版本的真源收敛到唯一一处（镜像），不会与「面板内更新」互相打架。也因此，**启动器恢复、版本审计、面板更新拦截等补丁都不需要**——这是「不对抗上游」换来的直接红利。
